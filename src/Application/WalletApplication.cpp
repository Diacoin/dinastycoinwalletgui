...
const char BYTECOIN_URI_SCHEME_NAME[] = "dinastycoin";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("dinastycoinwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("dinastycoinwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] dinastycoin wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "dinastycoin wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have dinastycoind with non-default RPC port, you should terminate it and relaunch dinastycoinWallet\n"
      "or\n"
      "Set the Local deamon required port in dinastycoinWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
