...
const char DEFAULT_WALLET_FILE_NAME[] = "dinastycoin.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "poolmining1.dinastycoin.com:5555";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("dinastycoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("dinastycoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("dinastycoinwallet.desktop");
  ...
  res = autorunSettings.contains("dinastycoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("dinastycoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.dinastycoin.dinastycoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=dinastycoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("dinastycoinWallet", appPath);
    } else {
      autorunSettings.remove("dinastycoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\dinastycoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:dinastycoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\dinastycoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\dinastycoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("dinastycoinwallet.cfg"));
