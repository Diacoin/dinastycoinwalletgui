...
private:
  ...
  const QDir m_dinastycoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfdinastycoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _dinastycoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo dinastycoinBlockchainInfo() const;
